...
from utils.trainer_cls import BackdoorModelTrainer, PureCleanModelTrainer, all_acc, get_target_dataset_loader,given_dataloader_test
...
class NADModelTrainer(PureCleanModelTrainer):
    ...
    def train_epoch(self,args,trainloader,nets,optimizer,scheduler,criterions,epoch):
        ...
        for idx, (inputs, labels, original_index, poison_indicator, original_targets) in enumerate(trainloader):
            ...
            if args.model == 'vgg16_pretrained':
                outputs_s = snet(inputs)
                features_out_3 = list(snet.children())[:-2]  # Remove the fully connected layer
                modelout_3 = nn.Sequential(*features_out_3)
                modelout_3.to(args.device)
                activation3_s = modelout_3(inputs)
                # activation3_s = snet.features(inputs)
                ...
                output_t = tnet(inputs)
                features_out_3 = list(tnet.children())[:-2]  # Remove the fully connected layer
                modelout_3 = nn.Sequential(*features_out_3)
                modelout_3.to(args.device)
                activation3_t = modelout_3(inputs)
                # activation3_t = tnet.features(inputs)
...
class nad(defense):
    ...
    def mitigation(self):
        ...
        original_tacc = given_dataloader_test(student, target_test_dataloader, criterionCls, args.non_blocking, args.device)[0]['test_acc']
        ...
        logging.info("===End===")
        logging.info(f"nad original tacc:{original_tacc:.2f}")
