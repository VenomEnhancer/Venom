...
from utils.trainer_cls import ModelTrainerCLS_v2, BackdoorModelTrainer, Metric_Aggregator, given_dataloader_test, general_plot_for_epoch, get_target_dataset_loader
...
class FinePrune(defense):
    ...
    def set_args(self, parser):
        ...
        parser.add_argument('--is_couple', type=lambda x: str(x) in ['True', 'true', '1'])
        return parser
    ...
    def defense(self):
        ...
        target_test_dataloader = get_target_dataset_loader(args, data_clean_testset, bd_test_dataset_with_transform[0][1])
        original_tacc = given_dataloader_test(netC, target_test_dataloader, criterion, args.non_blocking, args.device)[0]['test_acc']
        ...
        for num_pruned in range(0, len(seq_sort), math.ceil(len(seq_sort) * args.once_prune_ratio)):
            ...
            test_tacc = given_dataloader_test(net_pruned, target_test_dataloader, criterion, args.non_blocking, args.device)[0]['test_acc']
            ...
            test_tacc_list.append(float(test_tacc))
        ...
        logging.info(f"End prune. Pruned {num_pruned}/{len(seq_sort)} test_acc:{test_acc:.2f}  test_asr:{test_asr:.2f}  test_ra:{test_ra:.2f}  test_tacc:{test_tacc:.2f} ")
        ...
        logging.info("===End===")
        logging.info(f"fp original tacc:{original_tacc:.2f}")
