...
def dataset_and_transform_generate(args):
    ...
    ### get train/test transform
    if args.dataset == 'cfp':
        # don‘t crop
        train_img_transform = get_transform(args.dataset, *(args.img_size[:2]), train=False)
        test_img_transform = get_transform(args.dataset, *(args.img_size[:2]), train=False)
    elif not args.dataset.startswith('test'):
    ...
    if (train_dataset_without_transform is None) or (test_dataset_without_transform is None):
        ...
        if args.dataset.startswith('test'):  # for test only
            ...
            train_dataset_without_transform = GTSRB(
                args.dataset_path,
                split = 'train',
                download=True,
            )
            test_dataset_without_transform = GTSRB(
                args.dataset_path,
                split = 'test',
                download=True,
            )
            ...
            logging.warning("For ImageNet, this script may not give optimal results.")
