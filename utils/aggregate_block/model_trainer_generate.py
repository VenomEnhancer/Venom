...
try:
    ...
    logging.warning("facenet fails to import")
...
def generate_cls_model(
    ...
    if model_name == 'resnet18':
        ...
        if kwargs.get("pretrained", False):
            net_from_imagenet = models.vgg16_bn(pretrained=True)  # num_classes = num_classes)
            net = models.vgg16_bn(num_classes=num_classes, **{k: v for k, v in kwargs.items() if k != "pretrained"})
            partially_load_state_dict(net, net_from_imagenet.state_dict())
        else:
            net = models.vgg16_bn(num_classes=num_classes, **kwargs)
