...
from utils.bd_img_transform.ada_blended import adaBlendedImageAttack
...
def bd_attack_img_trans_generate(args):
    ...
    if args.attack in ['badnet',]:
    ...
    elif args.attack == 'ada_blended':
        # train_bd_transform = general_compose([
        #     (transforms.Resize(args.img_size[:2]), False),
        #     (np.array, False),
        #     (adaBlendedImageAttack(train=True), True),
        #     (npToFloat32, False),
        #     (npClipAndToUint8,False),
        # ])
        # test_bd_transform = general_compose([
        #     (transforms.Resize(args.img_size[:2]), False),
        #     (np.array, False),
        #     (adaBlendedImageAttack(train=False), True),
        #     (npToFloat32, False),
        #     (npClipAndToUint8,False),
        # ])
        train_bd_transform = general_compose([
            ...
            (SSBA_attack_replace_version(
                replace_images=np.load(args.attack_train_replace_imgs_path)  # 'resource/ada_blended/cifar10_ada_blended_train_b1.npy'
            ), True),
            (npClipAndToUint8,False),
        ])
        test_bd_transform = general_compose([
            ...
            (np.array, False),
            (SSBA_attack_replace_version(
                replace_images=np.load(args.attack_test_replace_imgs_path)  # 'resource/ada_blended/cifar10_ada_blended_test_b1.npy'
            ), True),
            (npClipAndToUint8,False),
        ])

    elif args.attack == 'ada_patch':
        train_bd_transform = general_compose([
            (transforms.Resize(args.img_size[:2]), False),
            (np.array, False),
            (SSBA_attack_replace_version(
