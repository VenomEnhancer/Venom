...
class prepro_cls_DatasetBD_v2(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if cover_indicator is None:
            cover_indicator = np.zeros(len(full_dataset_without_transform))

        self.poison_indicator = poison_indicator
        self.cover_indicator = cover_indicator
    ...
    def prepro_backdoor(self):
        for selected_index in tqdm(self.original_index_array, desc="prepro_backdoor"):
            ...
            if self.cover_indicator[selected_index] == 1:
                img, label = self.dataset[selected_index]
                img = self.bd_image_pre_transform(img, target=label, image_serial_id=selected_index)
                self.set_one_bd_sample(
                    selected_index, img, label, label,cover=True
                )

    def set_one_bd_sample(self, selected_index, img, bd_label, label, cover = False):
    ...
    def __getitem__(self, index):
        ...
        if self.poison_indicator[original_index] == 1:
            # bd
            img, label, original_target = self.bd_data_container[original_index]
            poison_or_not = 1
        elif self.cover_indicator[original_index] == 1:
            # cover clean
            img, label, original_target = self.bd_data_container[original_index]
            poison_or_not = 0
        else:
