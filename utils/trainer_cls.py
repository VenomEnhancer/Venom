...
def get_target_dataset_loader(args, dataset, attack_target):
        indexs = [i for i, (_, label,*_) in enumerate(dataset) if label == attack_target]
        random.shuffle(indexs)
        train_target_dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, drop_last=False,
...
def plot_acc_like_metric_pure(
    ...
    if validate_list_for_plot(test_tacc_list, len_set):
        plt.plot(x, test_tacc_list, marker = "+", linewidth=2, label="Test TACC", linestyle = "-.")
    else:
        logging.warning("test_tacc_list contains None, or len not match")
...
class ModelTrainerCLS_v2():
    ...
    def set_with_dataloader(
        ...
        self.clean_train_dataset = clean_train_dataset
        self.args = args
...
class PureCleanModelTrainer(ModelTrainerCLS_v2):
    ...
    def train_with_test_each_epoch_on_mix(self,
        ...
        for epoch in range(total_epoch_num):
            ...
            target_metrics,_,_ = self.test_given_dataloader(self.test_dataloader_dict["target_test_dataloader"], verbose=1)
    ...
    def test_current_model(self, test_dataloader_dict, device = None,):
        ...
        target_metrics,*_ = self.test_given_dataloader(test_dataloader_dict["target_test_dataloader"], verbose=1)
        test_tacc = target_metrics["test_acc"]
...
class BackdoorModelTrainer(ModelTrainerCLS_v2):
    ...
    def register_layers(self):
        ...
        #### register model
        if not self.layers:
            self.layers = init_layers(self.args.model, self.model)
        # register target layer
        self.top_sim_channels = get_model_similar_path(self.args.model,self.args.dataset,self.args.sim_mode, self.args.attack_target,self.args)
    ...
    def remove_hooks(self):
        for hook in self.hooks:
            hook.remove()
        self.hooks = []
        if self.args.sim_lf == 'at':
            for hook in self.hooks_clean:
                hook.remove()
            self.hooks_clean = []

    def get_target_dataset_loader(self):
        indexs = [i for i, (_, label,*_) in enumerate(self.clean_train_dataset) if label == self.args.attack_target]
        random.shuffle(indexs)
        train_target_dataloader = DataLoader(self.clean_train_dataset, batch_size=self.args.batch_size, shuffle=False, drop_last=True,
                       pin_memory=self.args.pin_memory, num_workers=self.args.num_workers, sampler=torch.utils.data.sampler.SubsetRandomSampler(indexs))
        return train_target_dataloader
 
    def one_forward_backward_couple_optimized_auto(self, x, labels, device, verbose=0, train_target_dataloader=None):
        model = self.model
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):    
            assert self.args.sim_mode in get_single_deep_p1_list(), f'Mode not in list {self.args.sim_mode}'
    ...
    def couple_train_one_epoch_on_mix(self, verbose=0):
        ...
        train_target_dataloader = self.get_target_dataset_loader()
        
        for batch_idx in range(self.batch_num_per_epoch):
            x, labels, original_index, poison_indicator, original_targets  = self.get_one_batch()
            # couple training
            if self.args.mtl == 'auto':
                one_batch_loss, batch_predict = self.one_forward_backward_couple_optimized_auto(x, labels, self.device, verbose, train_target_dataloader)
            else:
    ...
    def train_with_test_each_epoch_on_mix(self,
        ...
        for epoch in range(total_epoch_num):
            ...
            if self.args.is_couple and torch.rand(1)<args.sim_p:
                train_epoch_loss_avg_over_batch, \
                train_epoch_predict_list, \
                train_epoch_label_list, \
                train_epoch_original_index_list, \
                train_epoch_poison_indicator_list, \
                train_epoch_original_targets_list = self.couple_train_one_epoch_on_mix(verbose=1)
            else:
                train_epoch_loss_avg_over_batch, \
                train_epoch_predict_list, \
                train_epoch_label_list, \
